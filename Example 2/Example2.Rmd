---
title: "Dynamic Survival Analysis: Modelling the Hazard Function via Ordinary Differential Equations. rotterdam data"
...
This document provides codes and information about the *hazard-Response* model proposed in @christen:2023. 
...
q'(t) =  \beta q(t) \left( 1- \frac{q(t)}{\kappa} \right) -\alpha q(t) h(t)  ,  &  q(0) = q_0 \\ 
H'(t)  =  h(t), & H(0) = 0,
\end{cases}
$$

This model represents a particular version of the [competitive Lotka-Volterra equations](https://en.wikipedia.org/wiki/Competitive_Lotka%E2%80%93Volterra_equations), which is often used for modelling the growth on species in competition with harmful interactions. 


The following R code presents an example from @christen:2023 where we fit the hazard-Response model to the `rotterdam` data set from the R package `survival`. We present both Likelihood-based and Bayesian analyses.
